...
class ModelLoopModifier():
    ...
    def select_retrofit_scenarios(self, dict_rf=None): 
        ...
        if dict_rf is None:
            dict_rf = {2: 'default', 0: 'retr_1pc', 1: 'retr_2pc'}
    ...
    # def set_future_year(self, slct_ch, slct_ev, slct_tr, slct_hp, slct_rf, dict_fy=None):
    def set_future_year(self, slct_ch, slct_ev, slct_tr, slct_hp, slct_rf, slct_dpf, dict_fy=None): #slct_hp, slct_rf
        ...
        def set_dmnd(slct_ev, slct_hp, slct_rf, slct_dpf, str_fy=None):
            ...
            df_el = IO.param_to_df(self.ml.m.dmnd).loc[(IO.param_to_df(self.ml.m.dmnd).ca_id == 0) & ~(
                                                            IO.param_to_df(self.ml.m.dmnd).nd_id.isin(nd_arch_el_res+nd_arch_dsr+nd_arch_ev))]
            ...
            if slct_ev == 'basis': # Basis scenario for EV
                ...
                msg = ('Setting Electric car curves to residential for ' + str_fy.replace('_', ' ')
                      + ' from column {}'.format(slct_col) + ' for EV scenario ' + slct_ev ,  fy_val)
    
                logger.info(msg)
                    
                # Load before as we need for EV
                path_prof_id = os.path.join(config.PATH_CSV, 'def_profile.csv')
                path_dmnd = os.path.join(config.PATH_CSV, 'profdmnd.csv')
                ...
                list_nd_name_ev = self.ml.m.df_def_node.loc[self.ml.m.df_def_node.nd_id.isin(nd_arch_ev)].nd.to_list()
                dct_nd_id_ev = self.ml.m.df_def_node.loc[self.ml.m.df_def_node.nd_id.isin(nd_arch_ev)].set_index('nd')['nd_id'].to_dict()

                df_def_profile_ev = df_def_profile.copy().loc[(df_def_profile.pf.str.contains('EV'))&(df_def_profile.primary_nd.isin(list_nd_name_ev))] #here have to put EV
                # df_def_profile_ev = df_def_profile.copy().loc[df_def_profile.pf.str.contains('EV')] #here have to put EV
                df_def_profile_ev['nd_id'] = df_def_profile_ev['primary_nd'].map(dct_nd_id_ev)
                
                # df_dmnd_ev = df_dmnd.copy().loc[df_dmnd.dmnd_pf_id.isin(df_def_profile_ev.pf_id.to_list())].reset_index(drop=True)
                df_dmnd_ev = df_dmnd.copy().loc[df_dmnd.dmnd_pf_id.isin(df_def_profile_ev.pf_id.to_list())].reset_index(drop=True).rename(columns={'value':'diff_erg'})
                df_dmnd_ev = pd.merge(df_dmnd_ev,df_def_profile_ev,left_on='dmnd_pf_id', right_on='pf_id')
                
        
                dict_tm_soy_hour = self.ml.m.df_tm_soy_full.loc[self.ml.m.df_tm_soy_full.tm_id==0][['sy','hy']].set_index('hy')['sy'].to_dict()               
                # dict_tm_soy_hour = self.ml.m.df_tm_soy_full
                df_dmnd_ev['sy'] = df_dmnd_ev['hy'].map(dict_tm_soy_hour)
                # df_dmnd_ev['sy'] = df_dmnd_ev[['hy','doy']].map(dict_tm_soy_hour['hy'])
                df_ev = IO.param_to_df(self.ml.m.dmnd).loc[(IO.param_to_df(self.ml.m.dmnd).ca_id == 0) & (IO.param_to_df(self.ml.m.dmnd).nd_id.isin(nd_arch_ev))]
                # df_dmnd_ev['ca_id'] = 0
                # df_dmnd_ev = df_dmnd_ev.dropna()
                if fy_val == 2015:
                    
                    df_el_res_ev = pd.merge(df_ev,df_dmnd_ev,on=['sy','nd_id']).assign(
                                               value_new = lambda x: x.diff_erg * 0)
                    dct_dmnd_res_ev = (df_el_res_ev.set_index(['sy', 'nd_id', 'ca_id'])['value_new'].to_dict())
                    # df_el_res_ev = df_dmnd_ev[['hy','nd_id', 'ca_id', 'diff_erg']].assign(
                    #                            value_new = lambda x: x.diff_erg * 0)  
                    # dct_dmnd_res_ev = (df_el_res_ev.set_index(['hy', 'nd_id', 'ca_id'])['value_new'].to_dict())
    
                    for kk, vv in dct_dmnd_res_ev.items():
                          self.ml.m.dmnd[kk] = vv
                    
                elif fy_val == 2020:
                    
                    df_el_res_ev = pd.merge(df_ev,df_dmnd_ev,on=['sy','nd_id']).assign(
                                            value_new = lambda x: x.diff_erg * 5.830284714)
                    ...
                    df_el_res_ev = pd.merge(df_ev,df_dmnd_ev,on=['sy','nd_id']).assign(
                                            value_new = lambda x: x.diff_erg * 22.1720089)
                    ...
                    df_el_res_ev = pd.merge(df_ev,df_dmnd_ev,on=['sy','nd_id']).assign(
                                            value_new = lambda x: x.diff_erg * 55.53867249)
                    dct_dmnd_res_ev = (df_el_res_ev.set_index(['sy', 'nd_id', 'ca_id'])['value_new'].to_dict())
                    ...
                    df_el_res_ev = pd.merge(df_ev,df_dmnd_ev,on=['sy','nd_id']).assign(
                                            value_new = lambda x: x.diff_erg * 106.4236385)                   
                    dct_dmnd_res_ev = (df_el_res_ev.set_index(['sy', 'nd_id', 'ca_id'])['value_new'].to_dict())
                    ...
                    df_el_res_ev = pd.merge(df_ev,df_dmnd_ev,on=['sy','nd_id']).assign(
                                            value_new = lambda x: x.diff_erg * 167.8407792)                    
                    dct_dmnd_res_ev = (df_el_res_ev.set_index(['sy', 'nd_id', 'ca_id'])['value_new'].to_dict())
                    ...
                    df_el_res_ev = pd.merge(df_ev,df_dmnd_ev,on=['sy','nd_id']).assign(
                                            value_new = lambda x: x.diff_erg * 233.3600357)                    
                    dct_dmnd_res_ev = (df_el_res_ev.set_index(['sy', 'nd_id', 'ca_id'])['value_new'].to_dict())
                    ...
                    df_el_res_ev = pd.merge(df_ev,df_dmnd_ev,on=['sy','nd_id']).assign(
                                            value_new = lambda x: x.diff_erg * 302.9374627)                    
                    dct_dmnd_res_ev = (df_el_res_ev.set_index(['sy', 'nd_id', 'ca_id'])['value_new'].to_dict())
                ...
                msg = ('Setting Electric car curves to residential for ' + str_fy.replace('_', ' ')
                         + ' from column {}'.format(slct_col) + ' for EV scenario ' + slct_ev)
                ...
                if fy_val == 2015:                       
                       ...
                       df_el_res_ev = pd.merge(df_ev,df_dmnd_ev,on=['sy','nd_id']).assign(
                                               value_new = lambda x: x.diff_erg * 25.19629004)
                       ...
                       df_el_res_ev = pd.merge(df_ev,df_dmnd_ev,on=['sy','nd_id']).assign(
                                               value_new = lambda x: x.diff_erg * 76.18225986)
                       ...
                       df_el_res_ev = pd.merge(df_ev,df_dmnd_ev,on=['sy','nd_id']).assign(
                                               value_new = lambda x: x.diff_erg * 170.9707094)
                       ...
                       df_el_res_ev = pd.merge(df_ev,df_dmnd_ev,on=['sy','nd_id']).assign(
                                               value_new = lambda x: x.diff_erg * 299.0384097)
                       ...
                       df_el_res_ev = pd.merge(df_ev,df_dmnd_ev,on=['sy','nd_id']).assign(
                                               value_new = lambda x: x.diff_erg * 423.4295553)
                       ...
                       df_el_res_ev = pd.merge(df_ev,df_dmnd_ev,on=['sy','nd_id']).assign(
                                               value_new = lambda x: x.diff_erg * 521.9884918)
                ...
                df_def_profile_ev = df_def_profile.copy().loc[(df_def_profile.pf.str.contains('EV'))&(df_def_profile.primary_nd.isin(list_nd_name_ev))] #here have to put EV
                df_def_profile_ev['nd_id'] = df_def_profile_ev['primary_nd'].map(dct_nd_id_ev)
                df_def_profile_ev = df_def_profile_ev.loc[df_def_profile_ev.pf.str.contains('EV')]
                ...
                if fy_val == 2015:
                    ...
                    df_el_res_ev = pd.merge(df_ev,df_dmnd_ev,on=['sy','nd_id']).assign(
                                            value_new = lambda x: x.diff_erg * 45.53000509)
                    ...
                    df_el_res_ev = pd.merge(df_ev,df_dmnd_ev,on=['sy','nd_id']).assign(
                                            value_new = lambda x: x.diff_erg * 121.7429501)
                    ...
                    df_el_res_ev = pd.merge(df_ev,df_dmnd_ev,on=['sy','nd_id']).assign(
                                            value_new = lambda x: x.diff_erg * 234.867546)
                    ...
                    df_el_res_ev = pd.merge(df_ev,df_dmnd_ev,on=['sy','nd_id']).assign(
                                            value_new = lambda x: x.diff_erg * 384.9037931)
                    ...
                    df_el_res_ev = pd.merge(df_ev,df_dmnd_ev,on=['sy','nd_id']).assign(
                                            value_new = lambda x: x.diff_erg * 571.8516911)
                    ...
                    df_el_res_ev = pd.merge(df_ev,df_dmnd_ev,on=['sy','nd_id']).assign(
                                            value_new = lambda x: x.diff_erg * 795.670272)
                ...
                df_def_profile_ev = df_def_profile.copy().loc[(df_def_profile.pf.str.contains('EV'))&(df_def_profile.primary_nd.isin(list_nd_name_ev))] #here have to put EV
                df_def_profile_ev['nd_id'] = df_def_profile_ev['primary_nd'].map(dct_nd_id_res)
                df_def_profile_ev = df_def_profile_ev.loc[df_def_profile_ev.pf.str.contains('EV')]
                ...
                if fy_val == 2015:
                    ...
                    df_el_res_ev = pd.merge(df_ev,df_dmnd_ev,on=['sy','nd_id']).assign(
                                            value_new = lambda x: x.diff_erg * 50.93637298)
                    ...
                    df_el_res_ev = pd.merge(df_ev,df_dmnd_ev,on=['sy','nd_id']).assign(
                                            value_new = lambda x: x.diff_erg * 299.4442147)
                    ...
                    df_el_res_ev = pd.merge(df_ev,df_dmnd_ev,on=['sy','nd_id']).assign(
                                            value_new = lambda x: x.diff_erg * 669.6658585)
                    ...
                    df_el_res_ev = pd.merge(df_ev,df_dmnd_ev,on=['sy','nd_id']).assign(
                                            value_new = lambda x: x.diff_erg * 779.0235069)
                    ...
                    df_el_res_ev = pd.merge(df_ev,df_dmnd_ev,on=['sy','nd_id']).assign(
                                            value_new = lambda x: x.diff_erg * 793.7439832)
...
#                self.ml.m.fc_cp_ann[kk] = vv * 0.01
        ...
        # def set_ev_capacity(slct_ev, str_fy=None):
        #     ''' Select electric vehicle scenario for archetypes'''
        ...
        #     msg = ('Setting cap_pwr_leg to values for EV' + str_fy.replace('_', ' ') +
        #                ' from column {}'.format(slct_col) + ' for scenario ' + slct_ev)
        #     logger.info(msg)
...
#       set_ev_capacity(slct_ev, str_fy)        
