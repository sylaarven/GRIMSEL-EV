...
class ModelLoopModifier():
    ...
    def set_future_year(self, slct_ch, slct_tr, slct_hp, slct_rf, dict_fy=None): #slct_hp, slct_rf
        ...
        list_ht_arch = list_hp_arch + list_ht_sto_arch
        
        list_all_arch = list_sto_pho_arch + list_ht_arch
        ...
        #######################################################################
        def set_dmnd(slct_hp, slct_rf, str_fy=None):
...
#            logger.info(msg)
            ...
            dct_pf_id_dmnd = self.ml.m.df_node_encar.loc[self.ml.m.df_node_encar.ca_id.isin([1,2])].set_index('dmnd_pf_id')['nd_id'].to_dict()
            list_pf_id_dmnd = self.ml.m.df_node_encar.loc[self.ml.m.df_node_encar.ca_id.isin([1,2])]['dmnd_pf_id'].to_list()
            dict_tm_soy = self.ml.m.df_tm_soy_full.loc[self.ml.m.df_tm_soy_full.tm_id==0][['sy','doy']].set_index('doy')['sy'].to_dict()
            
            df_ht_other = self.ml.m.df_profdmnd.loc[self.ml.m.df_profdmnd.dmnd_pf_id.isin(list_pf_id_dmnd)].copy()
            df_ht_other['nd_id'] = df_ht_other['dmnd_pf_id'].map(dct_pf_id_dmnd)
            ...
            if slct_rf == 'default':
                ...
                msg = ('Setting special heating dmnd_sum to values' + str_fy.replace('_', ' ')
                   + ' from column {}'.format(slct_col) + ' for retrofit scenrio ' + slct_rf)
...
#                df_ht_fossil = self.ml.m.df_profdmnd.loc[self.ml.m.df_profdmnd.dmnd_pf_id.isin(list_pf_id_dmnd)].copy()
#                df_ht_fossil['nd_id'] = df_ht_fossil['dmnd_pf_id'].map(dct_pf_id_dmnd)
#                df_ht_fossil['sy'] = df_ht_fossil['doy'].map(dict_tm_soy)
#                df_ht_fossil['ca_id'] = 1
#                
#                df_ht_fossil = df_ht_fossil[['nd_id', 'sy','erg_tot_fossil','ca_id']].join(scaling_factor_ht, on=scaling_factor_ht.index.names)
                ...
                df_ht_retr_1pc = self.ml.m.df_profdmnd.loc[self.ml.m.df_profdmnd.dmnd_pf_id.isin(list_pf_id_dmnd)].copy()
                df_ht_retr_1pc['nd_id'] = df_ht_retr_1pc['dmnd_pf_id'].map(dct_pf_id_dmnd)
                ...
                df_ht_retr_2pc = self.ml.m.df_profdmnd.loc[self.ml.m.df_profdmnd.dmnd_pf_id.isin(list_pf_id_dmnd)].copy()
                df_ht_retr_2pc['nd_id'] = df_ht_retr_2pc['dmnd_pf_id'].map(dct_pf_id_dmnd)
...
#                self.ml.m.fc_cp_ann[kk] = vv * 0.01
        ...
        def set_hp_capacity(slct_hp, slct_rf, str_fy=None):
            ...
            if slct_rf == 'default':
                ...
                dct_caphp_aw = (self.ml.m.df_plant_encar.loc[self.ml.m.df_plant_encar.pp_id.isin(list_hp_aw)]
                                 .set_index(['pp_id', 'ca_id'])[slct_col]
                                 .to_dict())
                
                dct_caphp_ww = (self.ml.m.df_plant_encar.loc[self.ml.m.df_plant_encar.pp_id.isin(list_hp_ww)]
                                 .set_index(['pp_id', 'ca_id'])[slct_col]
                                 .to_dict())
                ...
                if slct_hp == 'full': # full heat pumps in 2050
                    ...
                    for kk, vv in dct_caphp_aw.items():
                        self.ml.m.cap_pwr_leg[kk] = vv
                        
                    for kk, vv in dct_caphp_ww.items():
                        self.ml.m.cap_pwr_leg[kk] = vv
                    ...
                    dct_caphp_aw_fossil = (self.ml.m.df_plant_encar.loc[self.ml.m.df_plant_encar.pp_id.isin(list_hp_aw)]
                                 .set_index(['pp_id', 'ca_id'])[slct_col_fossil]
                                 .to_dict())
                
                    dct_caphp_ww_fossil = (self.ml.m.df_plant_encar.loc[self.ml.m.df_plant_encar.pp_id.isin(list_hp_ww)]
                                 .set_index(['pp_id', 'ca_id'])[slct_col_fossil]
                                 .to_dict())
                    for kk, vv in dct_caphp_aw_fossil.items():
                        self.ml.m.cap_pwr_leg[kk] = vv
                        
                    for kk, vv in dct_caphp_ww_fossil.items():
                        self.ml.m.cap_pwr_leg[kk] = vv
...
#        excl_fl = []
        ...
        set_cap_avlb(str_fy)
        set_dmnd(slct_hp, slct_rf, str_fy)
