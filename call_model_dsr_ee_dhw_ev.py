...
import model_loop_modifier_dsr_ee_dhw_ev as model_loop_modifier
...
sc_out = os.path.abspath('/users/output_files/')
...
nd_arch = df_def_node.loc[np.invert(df_def_node.nd.isin(['CH0', 'AT0', 'IT0', 'FR0', 'DE0']))].nd_id.tolist()
...
nd_arch_el = list(set(nd_arch) - set(nd_arch_ev) - set(nd_arch_ht) - set(nd_arch_dsr))
nd_arch_el_res = df_def_node.loc[df_def_node.nd.str.contains('SFH|MFH')].nd_id.tolist()
nd_arch_el_res = list(set(nd_arch_el_res) - set(nd_arch_ht))
...
nd_arch_el_res_str = df_def_node.loc[(df_def_node.nd.str.contains('SFH|MFH')) &~(df_def_node.nd.str.contains('EV')) &~(df_def_node.nd.str.contains('HT'))].nd.tolist()
...
# nhours_dict_ev = {nd : (3,3) for nd in nd_arch_ev_str} 
nhours_dict_ht = {nd : (24,24) for nd in nd_arch_ht_str} 
# nhours_dict_el = {nd: (1,1) for nd in nd_arch_not_ht_str}
#nhours_dict_el = {nd: (12,12) for nd in nd_arch_not_ht_str}
# nhours_dict_el = {nd: (12,12) for nd in nd_arch_not_ht_str}
# nhours_dict_el_res = {nd: (24,24) for nd in nd_arch_el_res_str}
# nhours_dict_el_res_12h = {nd: (12,12) for nd in nd_arch_el_res_str}
nhours_dict_dsr = {nd: (24,24) for nd in nd_arch_dsr_str}
# nhours_dict = dict(nhours_dict_ht,**nhours_dict_ev)
# nhours_dict = nhours_dict_ht
nhours_dict = dict(nhours_dict_dsr,**nhours_dict_ht)
...
mkwargs = {
        ...
        # [
                   ...
                   # 'SFH_URB_0_EV', #'SFH_RUR_0_EV', 'SFH_SUB_0_EV',
                    # 'SFH_URB_1_EV', 'SFH_RUR_1_EV', 'SFH_SUB_1_EV',
                       # 'SFH_URB_2_EV', 'SFH_RUR_2_EV', 'SFH_SUB_2_EV',
                       # 'SFH_URB_3_EV', 'SFH_RUR_3_EV', 'SFH_SUB_3_EV',
                   
                    # 'MFH_URB_0_EV', 'MFH_RUR_0_EV', 'MFH_SUB_0_EV',
                    # 'MFH_URB_1_EV', 'MFH_RUR_1_EV', 'MFH_SUB_1_EV',
                    # 'MFH_URB_2_EV', 'MFH_RUR_2_EV', 'MFH_SUB_2_EV',
                    # 'MFH_URB_3_EV', 'MFH_RUR_3_EV', 'MFH_SUB_3_EV',
                    # 'IND_RUR', 'IND_SUB', 
                   # 'IND_URB',
    ...
    'symbolic_solver_labels': True,  # Set to True for debugging (set name to pyomo indices in cplex files)
    'constraint_groups':
        MB.get_constraint_groups(excl=['supply']),  # BE CAREFUL WITH THIS
    #               'chp','chp_new']),
    'nthreads': 5,  # Only passed to CPLEX
    ...
    if nd in nd_nt:
        ...
        exports += (sum(get_transmission(sy, nd, nd_2, ca, True) * ((1 - self.grid_losses[nd, ca])**0.5)
                ...
                + sum(get_transmission(sy, nd, nd_2, ca, True)
                    / self.nd_weight[nd]
                  for (nd, nd_2, ca)
                  in set_to_list(self.ndcnn, [nd, nd_arch_ht + nd_arch_dsr + nd_arch_ev, ca])))
...
nd_arch_el_res = sorted(list(set(nd_arch_res) - set(nd_arch_ev) - set(nd_arch_ht) - set(nd_arch_dsr)))
...
def run_model(run_id):
    ...
    mlm.set_future_year(slct_ch=slct_ch, slct_ev=slct_ev, slct_tr=slct_tr, slct_hp=slct_hp, slct_rf=slct_rf, slct_dpf=slct_dpf)
    # mlm.set_future_year(slct_ch=slct_ch, slct_ev=slct_ev, slct_tr=slct_tr, slct_hp=slct_hp, slct_rf=slct_rf)
...
# run_parallel(ml, run_model, 3, groupby=['swev'], adjust_logger_levels=True)
# run_parallel(ml, run_model, 3, groupby=['swev','swhp','swrf','swdpf','swtr'], adjust_logger_levels=True)
# run_parallel(ml, run_model, 3, groupby=['swfy','swch','swev','swhp','swrf','swdpf','swtr'], adjust_logger_levels=True)
