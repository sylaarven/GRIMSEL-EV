...
import model_loop_modifier_res_heating as model_loop_modifier
...
##                                    ],
               # ['CH0',
                # 'SFH_SUB_2','SFH_SUB_HT_2'],#'MFH_RUR_2','MFH_RUR_HT_2'],
...
#                   ('hom', range(24))
           ...
           'constraint_groups': 
               MB.get_constraint_groups(excl=['ror','supply']),
#               'chp','chp_new']),
...
#df_cop_35 = df_def_cop_35.loc[df_def_cop_35.pp_id.isin([i for i in self.pp])]
    ...
    if nd in nd_nt:
        ...
        exports += (sum(get_transmission(sy, nd, nd_2, ca, True) * ((1 - self.grid_losses[nd, ca])**0.5)
                ...
                + sum(get_transmission(sy, nd, nd_2, ca, True)
                    / self.nd_weight[nd]
                  for (nd, nd_2, ca)
                  in set_to_list(self.ndcnn, [nd, nd_arch_ht, ca])))
...
nd_arch_ht = sorted(self.df_def_node.loc[self.df_def_node.nd.str.contains('HT')].nd_id.tolist())
nd_arch_res = sorted(self.df_def_node.loc[self.df_def_node.nd.str.contains('SFH|MFH')].nd_id.tolist())
nd_arch_el_res = sorted(list(set(nd_arch_res) - set(nd_arch_ht)))
...
print('RUNNING')
#run_parallel(ml, run_model, 3, groupby=['swhp'], adjust_logger_levels=True)
